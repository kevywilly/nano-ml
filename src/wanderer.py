...
class Wanderer(SingletonConfigurable):

    config = traitlets.Instance(TrainingConfig).tag(config=True)
    ...
    def __init__(self, robot: Robot, *args, **kwargs):
        ...
        self.model = config.load_model(pretrained=False)
        self.dir = 0
    ...
    def _update(self, change):
        ...
        if self.config.name.startswith("obstacle3d"):
